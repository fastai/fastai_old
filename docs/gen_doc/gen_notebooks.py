...
def get_md_cell(source, metadata=None):
    """
    Returns a markdown cell containing the source text.
    """
...
def get_empty_cell(ctype='markdown'):
    """
    Returns an empty cell of type ctype.
    """
...
def get_code_cell(code, hidden=False):
    """
    Returns a code cell containing the code.
    """
...
def get_doc_cell(mod_name, ft_name):
    """
    Returns a code cell with the command to show the doc of a given function.
    """
...
def is_enum(cls):
    """
    Returns True is cls is a enumerator class.
    """
    return cls == enum.Enum or cls == enum.EnumMeta

def get_inner_fts(elt):
    """
    Returns the inner functions of a class.
    """
    fts = []
...
def get_ft_names(mod):
    """
    Returns all the functions of a given module.
...
def execute_nb(fname):
    """
    Execute a give module.

    Warning: any module used in the notebook that isn't inside must be in the same directory
    as this script.
    """
...
def generate_all(pkg_name, dest_path, exclude=['.ipynb_checkpoints', '__pycache__']):
    """
...
def read_nb_content(nb, mod_name):
    """
    Builds a dictionary containing the position of the cells giving the document for functions
    in a notebook.
    """
    doc_fns = {}
...
def insert_cells(cells, pos_dict, mod_name, ft_name):
    """
    Insert the function doc cells of a function in the list of cells at their correct postition
    and updates the position dictionary.
    """
