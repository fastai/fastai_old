...
def test_listify():
    assert core.listify(5,1)        == [5]
    assert core.listify(5, [1,1])   == [5, 5]
    assert core.listify([5],1)      == [5]
    assert core.listify([5], [1,1]) == [5, 5]
    assert core.listify("ab", "cd")           == ["a", "b"]
    assert core.listify("ab", ["cd", "ef"])   == ["a", "b"]
    assert core.listify(["ab"], "cd")         == ["ab", "ab"]
    assert core.listify(["ab"], ["cd", "ef"]) == ["ab", "ab"]
