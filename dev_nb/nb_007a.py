        ...
        #################################################
        # file to edit: /data1/jhoward/git/fastai_v1/dev_nb/007a_imdb_preparation.ipynb
...
class Vocab():
    ...
    def create(cls, path, tokens, max_vocab, min_freq):
        ...
        with open(path/'numericalize.log','w') as f: f.write(str(hash(frozenset(itos))))
        return cls(path)
...
class TextDataset():
    ...
    def check_ids(self):
        ...
        with open(self.id_files[2]) as f:
            if hash(frozenset(itos)) != int(f.read()) or len(itos) > self.max_vocab + 2: return False
