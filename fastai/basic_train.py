...
def fit(epochs:int, model:nn.Module, loss_fn:tc.LossFunction, opt:OptimWrapper, data:DataBunch, 
        callbacks:Collection[Callback]=None, metrics:Collection[tc.Metric]=None, pbar:Callable=None):
...
class Learner():
    ...
    def fit(self, epochs:int, lr:c.Floats, wd:c.Floats=0., callbacks:Collection[Callback]=None):
    ...
    def create_opt(self, lr:c.Floats, wd:c.Floats=0.):
        if self.layer_groups is None: self.layer_groups = [self.model]
        lrs = c.listify(lr, self.layer_groups)
